"""amp_leung_nmcnr — Leung NMCNR, 3-stage NMC + nulling resistor.

Sibling of amp_leung_nmcf (same bias band + cascoded stage 1 + stage 2);
deltas per netlist: xm11 is a 10x net013-biased class-A pull-up on VOUT
(no gmf2 feedforward), and the compensation is net050 -C0- net044 -R0-
VOUT with the inner cap C1 tapping net044 (before the nulling resistor).
"""
import sys
...
# ─────────────────────────────────────────────────────────────────────────────
def draw():
    sh = Sheet(AMP_NL / 'Leung_NMCNR_Pin_3')
    d = sh.d
    ...
    sh.pfet('xm4', XTAIL, 'M4 (4x)', bias='net013', left=True)
    ...
    m10 = sh.pfet('xm10', X2A, 'M10', left=True)
    ...
    # stage 3: gm3 NMOS + 10x class-A bias pull-up (no feedforward here)
    sh.pfet('xm11', X3, 'M11 (10x)', bias='net013')
    ...
    gate_feed(sh, (X2B, 4.4), g23.x, g23.y)      # net049 -> M23 gate

    # nested Miller with nulling resistor:
    # net050 -C0- net044 -R0- VOUT, inner C1 taps net044
    sh.cap('c0', (XCR, 6.3), (XN, 6.3), 'C0')
    dot(d, (XCR, 6.3))
    sh.res('r0', (XN, 6.3), (X3, 6.3), 'R0')
    dot(d, (X3, 6.3))
    sh.cap('c1', (X2B, 5.2), (XN, 5.2), 'C1')
    dot(d, (X2B, 5.2))
    wire(d, (XN, 5.2), (XN, 6.3))
    dot(d, (XN, 6.3))
    node_label(d, (XN + 0.22, 5.45), 'net044')

    title(d, (XB0 + XMAX) / 2, Y_VDD + 0.8,
          'Leung NMCNR — 3-stage nested-Miller + nulling resistor '
          '(SKY130, 1.8 V)')
    sh.save(KEY)
