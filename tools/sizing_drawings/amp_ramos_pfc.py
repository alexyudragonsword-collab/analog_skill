"""amp_ramos_pfc — Ramos PFC, 3-stage positive-feedback compensation.

Same skeleton as Leung NMCF with two twists straight from the netlist:
the bias diode / distribution node is called net1 (not net013), and the
inner cap C1 runs net050 -> net049 (between the first- and second-stage
outputs — the positive-feedback path) instead of nesting to VOUT.
C0 stays net050 -> VOUT; the feedforward PMOS is the gmf1 device.
"""
import sys
...
# ─────────────────────────────────────────────────────────────────────────────
def draw():
    sh = Sheet(AMP_NL / 'Ramos_PFC_Pin_3')
    d = sh.d
    ...
    sh.pfet('xm4', XTAIL, 'M4 (4x)', bias='net1', left=True)
    ...
    # stage 3: gm3 NMOS + gmf1 PMOS feedforward into VOUT
    m11 = sh.pfet('xm11', X3, 'M11 (gmf1)', left=True)
    ...
    # compensation: C0 net050 -> VOUT; C1 net050 -> net049 (the
    # positive-feedback cap between the stage-1 and stage-2 outputs)
    sh.cap('c0', (XCR, 6.3), (X3 - 0.9, 6.3), 'C0')
    wire(d, (X3 - 0.9, 6.3), (X3, 6.3))
    dot(d, (XCR, 6.3)); dot(d, (X3, 6.3))
    sh.cap('c1', (XCR, 5.2), (X2B - 0.9, 5.2), 'C1')
    wire(d, (X2B - 0.9, 5.2), (X2B, 5.2))
    dot(d, (XCR, 5.2)); dot(d, (X2B, 5.2))

    title(d, (XB0 + XMAX) / 2, Y_VDD + 0.8,
          'Ramos PFC — 3-stage positive-feedback compensation '
          '(SKY130, 1.8 V)')
    sh.save(KEY)
