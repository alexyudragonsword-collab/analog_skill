"""amp_leung_nmcf — Leung NMCF, 3-stage nested-Miller + feedforward.

Reference figure for the NMC family: bias band (net013/VB3/VB4 columns),
tail left of the cascoded input stage, mirror gates inward, labelled bias
stubs, Miller caps in the 5.2-6.3 horizontal band.
"""
import sys
...
# ─────────────────────────────────────────────────────────────────────────────
def draw():
    sh = Sheet(AMP_NL / 'Leung_NMCF_Pin_3')
    d = sh.d
    ...
    # (label sits left by default — fine at the wider spacing)
    ...
    g23 = m23.absanchors['gate'], m23.absanchors['gate']
    gate_feed(sh, (X2B, 4.4), g23[0].x, g23[0].y)  # net049 -> M23 gate

    # nested Miller caps
    sh.cap('c0', (XCR, 6.3), (X3 - 0.9, 6.3), 'C0')
    wire(d, (X3 - 0.9, 6.3), (X3, 6.3))
    dot(d, (XCR, 6.3)); dot(d, (X3, 6.3))
    sh.cap('c1', (X2B, 5.2), (X3 - 0.9, 5.2), 'C1')
    wire(d, (X3 - 0.9, 5.2), (X3, 5.2))
    dot(d, (X2B, 5.2)); dot(d, (X3, 5.2))

    title(d, (XB0 + XMAX) / 2, Y_VDD + 0.8,
          'Leung NMCF — 3-stage nested-Miller + feedforward '
          '(SKY130, 1.8 V)')
    sh.save(KEY)
