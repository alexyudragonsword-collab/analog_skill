#!/usr/bin/env python3
"""AC open-loop gain/phase simulation for the two-stage op amp."""
...
def simulate_ac():
    print("\n=== Two-Stage Op Amp AC Gain/Phase Sweep ===")
    dut = render_dut()
    paths = {
        "freq": LOG_DIR / "opamp_ac_freq.txt",
        "gain_db": LOG_DIR / "opamp_ac_gain_db.txt",
        "phase": LOG_DIR / "opamp_ac_phase.txt",
    }
    kw = dict(
        **common_kw(dut),
        ac_pts=AC_PTS,
    ...
    rc = run_netlist("testbench_opamp_ac.cir.tmpl", kw, "opamp_ac.log")
    ...
    if freq is not None and gain_db is not None:
        ...
        if phase_deg is not None and not np.isnan(ugb_hz):
            ...
            phase_margin_deg = 180.0 + phase_ugb_deg
        metrics = {
        ...
        print(f"  [ac] estimated PM = {phase_margin_deg:.1f} deg")
