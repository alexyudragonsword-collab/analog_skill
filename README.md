...
| **Sizing** | Automatic device sizing over **27 circuits**: 20 SKY130 designs from the vendored [AnalogGym](https://github.com/CODA-Team/AnalogGym) subset (15 published three-stage Miller op amps + Basic LDO + 4 LDO variants), 1 original current-mirror OTA, and 6 PTM circuit-skills entries. Editable bounds and metric targets with **hard constraints**; optimizers: **CMA-ES** (the measured default: feasible on 12 of 18 circuit-seed rows against DE's 6), differential evolution (plus a Powell polish and a constrained form), built-in Sobol+Powell, Optuna TPE, two **LLM-driven** loops, and an **AI finish** behind CMA-ES or DE — the search stalls, the model names the fix, a line search lands it, the search resumes from there (measured at three seeds on nine circuits: feasible on 20 of 27 circuit-seed rows against CMA-ES alone's 18); parallel evaluation; convergence curve, before/after wave overlays, per-device change summary, run history, an **archive of every evaluation** with a warm start from the best point under the current targets (beats a cold search at twice the evaluations on 23 of 24 target-set-seed rows), **metric models** trained on that archive that propose candidates for new targets in seconds, and import of **your own SKY130 amplifier**. |
