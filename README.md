...
| **Sizing** | Automatic device sizing over **27 circuits**: 20 SKY130 designs from the vendored [AnalogGym](https://github.com/CODA-Team/AnalogGym) subset (15 published three-stage Miller op amps + Basic LDO + 4 LDO variants), 1 original current-mirror OTA, and 6 PTM circuit-skills entries. Editable bounds and metric targets with **hard constraints**; optimizers: built-in Sobol+Powell, differential evolution (plus a Powell polish, a constrained form and a seed portfolio), CMA-ES with restarts, Optuna TPE, two **LLM-driven** loops, and **DE + AI finish** — DE searches, the model names the fix once, a line search lands it (the only one that has met every target on the reference amplifier); parallel evaluation; convergence curve, before/after wave overlays, per-device change summary, run history, and import of **your own SKY130 amplifier**. |
