...
**Windows 下首次启动较慢，且会被 SmartScreen 拦截。** 产物未做代码签名，
SmartScreen 会提示"Windows 已保护你的电脑"—— 点*更多信息* → *仍要运行*。
之后首次运行会先解压随包资源（单文件版约 100 MB）才出现窗口，冷盘上需要一
会儿，启动画面会说明这一点。如果**完全没有反应**，去 `%TEMP%` 目录找
`AnalogStudio-crash.txt` —— 应用会把失败原因写在那里，而不是静默退出。
