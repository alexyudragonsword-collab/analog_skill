...
def schematic_path(circuit: str) -> Path | None:
    """Pre-rendered schematic PNG for a CIRCUITS key (None if missing).

    The PNGs are generated by tools/gen_schematics.py and shipped in
    app/resources/schematics (bundled as app_resources when frozen).
    """
    p = paths.resources_dir() / 'schematics' / f'{circuit}.png'
